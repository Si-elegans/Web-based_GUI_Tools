...
var renderWidth;
var renderHeight;
var experimentDuration, locomotionDuration;
...
function init() {
	...
	var screenDom = document.getElementById("visualizationScreen");

	transformControls = new THREE.OrbitControls( camera, screenDom);
	...
	document.getElementById("visualizationScreen").appendChild(container);
...
function goToAnimation(time) {
	...
	if(time > locomotionDuration){
		time = locomotionDuration;
		currentStep = locomotion.length - 1;
		timeFromCurrentStep = 0;
	}
	var playTimeText = document.getElementById("playTimeText");
	playTimeText.value = (time + "s");
	RVAPIFocusOnTimeLine(time,time+3*playingVelocity,playingVelocity)
...
function loadTimeline(experiment){
	...
	//set play/pause slider's length
	experimentDuration = experiment.experimentDuration/1000;
	...
	var self = this;
		...
		// create the timeline
		var timelineContainer = this.$("#visualizationTimeline");
				...
				if (aux[i] != "") {
					$.ajax({
						...
						success : function (data2) {
							...
							locomotionDuration = locomotion.length * timeStep;
